 ...
 * site's origin, so storing the uploaded bytes verbatim is stored XSS reachable by anyone
 * allowed to add media. SVG stays accepted rather than rejected — logos are the common
 * case and blocking them breaks real sites — so it is cleaned on the way in.
 *
 * Allow-list by design: anything not known to be safe is dropped, so a novel vector fails
